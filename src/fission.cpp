...
namespace mut
{
    ...
    constexpr float alpha{1.1};

    ///returns on personal reproduction
    constexpr float beta{1.1};

    constexpr bool output_returns{true};
    ...
    constexpr bool output_rel{false};
...
void get_averages(vector<organism>& m_population, long generations)
{
    ...
    if (mut::invasion)
    {
        ...
        outfile << fit_freq << '\t' << suppress_freq << endl;
    }
