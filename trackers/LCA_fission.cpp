...
public:
    ...
    // codes vector for finding last common ancestor
    void set_code_vector()
    {
        codes.assign(mut::n_codes, 0.5);
    }
...
public:
    ...
    void check_codes(vector<int>& smalls, vector<int>& bigs)
    {
        int small = mut::n_codes;
        int big = 0;

        vector<double> an_codes = m_current[0].get_mut_codes();
        vector<double> an_codes2{};
        int x = m_current.size();
        for (int i = 1; i < x; ++i)
        {
            vector<double> c_list = m_current[i].get_codes();
            for (double i : c_list)
            {
                auto it = find(an_codes.begin(), an_codes.end(), i);
                if ( it != an_codes.end())
                {
                    an_codes2.push_back(*it);
                }
        ...
        // cout << "NUMBER OF CODES: " << an_codes.size() << endl;


        vector<double> unmut_codes = m_current[0].get_codes();
        for (double i : an_codes)
        {
            auto it = find(unmut_codes.begin(), unmut_codes.end(), i);
            int dist = distance(unmut_codes.begin(), it);
...
void track_codes(vector<organism>& m_population)
{
    vector<int> bigs;
    vector<int> smalls;
    ...
    for (int i = 0; i < x; ++i)
    {
        m_population[i].check_codes(smalls, bigs);
    }
    int length=0;
    for (int i : bigs)
    {
    ...
    average = average / (double)(smalls.size());

    string var_name = to_string(mut::threshold) + "codes.dat";
    ofstream outfile;
    outfile.open(var_name, ios::app);
    outfile << length << '\t' << max << average << endl;
...
public:
    ...
    void process()
    {
        ...
        while (time_steps > 0)
        {
            ...
            if (time_steps % mut::code_freq == 0)
            {
                track_codes(m_population);
            }
